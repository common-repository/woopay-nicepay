...
if ( ! class_exists( 'WooPayNicePayBase' ) ) {
	class WooPayNicePayBase extends WooPayNicePay {
		...
		private function woopay_init() {
			...
			add_action( 'add_woopay_nicepay_virtual_bank_information', array( $this, 'add_virtual_information' ), 10, 2 );
			add_action( 'woopay_nicepay_virtual_bank_schedule', array( $this, 'woopay_cancel_unpaid_virtual_bank_orders' ) );
			...
			// Classes
			include_once( 'class-woopay-nicepay-actions.php' );
			include_once( 'class-woopay-nicepay-payment.php' );
			include_once( 'class-woopay-nicepay-refund.php' );

			// API
			include_once( 'class-woopay-nicepay-api.php' );

			// Payment Methods
			include_once( 'methods/class-woopay-nicepay-card.php' );
			include_once( 'methods/class-woopay-nicepay-transfer.php' );
			include_once( 'methods/class-woopay-nicepay-virtual.php' );
			include_once( 'methods/class-woopay-nicepay-mobile.php' );
		}
		...
		public function woopay_add_email_class( $emails ) {			
			$emails[ 'WooPayNicePayEmail' ] = include( 'class-woopay-nicepay-email.php' );
		...
		public function woopay_cancel_unpaid_virtual_bank_orders() {
			...
			$virtual_bank_settings = get_option( 'woocommerce_nicepay_virtual_settings', null );
			...
			if ( $unpaid_orders ) {
				foreach ( $unpaid_orders as $unpaid_order ) {
					...
					$expiry_date = get_post_meta( $unpaid_order, '_woopay_nicepay_expirydate', true );
		...
		public function for_translation() {
			array(
				__( 'Korean Payment Gateway integrated with NicePay for WooCommerce.', $this->woopay_domain )
			);
		}
	}

	return new WooPayNicePayBase();
}